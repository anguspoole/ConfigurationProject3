...
void ProcessAsyncKeys(GLFWwindow* window)
{
	...
	if ( AreAllModifiersUp(window) )
	{
		...
		cMeshObject * shipSphere = findObjectByFriendlyName("ShipSphere");

		if ( glfwGetKey( window, GLFW_KEY_W ) == GLFW_PRESS )
		{
			shipSphere->position.z += 0.3f;
...
//			::g_pFlyCamera->Roll_CW_CCW( -cameraSpeed );
	...
	{	
		cMeshObject * houseSphere = findObjectByFriendlyName("HouseSphere");
		if ( glfwGetKey( window, GLFW_KEY_W ) == GLFW_PRESS )
		{
			houseSphere->position.z += 0.3f;
		}
		if ( glfwGetKey( window, GLFW_KEY_S ) == GLFW_PRESS )	// "backwards"
		{	
			houseSphere->position.z -= 0.3f;
