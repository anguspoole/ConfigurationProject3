...
// If this is true, then we are animating the top water
uniform bool bOffsetWater;

// If this is true, then we are animating the bottom water
uniform bool bOffsetWater2;

uniform int bShaderMode;
...
//		materialDiffuse = objectDiffuse;
		...
		if(bOffsetWater)
		{
			vec2 newVerts = vertUV_x2.st;
			newVerts.x += waterOffset.x;
			newVerts.y += waterOffset.y;
			
			tex0Col = texture( texture00, newVerts ).rgba;
			tex1Col = texture( texture01, newVerts ).rgba;
			tex2Col = texture( texture02, newVerts ).rgba;
			tex3Col = texture( texture03, newVerts ).rgba;
			tex4Col = texture( texture04, newVerts ).rgba;
			tex5Col = texture( texture05, newVerts ).rgba;
			tex6Col = texture( texture06, newVerts ).rgba;
			tex7Col = texture( texture07, newVerts ).rgba;

		
			//tex0Col = texture( texture00, vertUV_x2.s + waterOffset.x, vertUV_x2.t + waterOffset.y ).rgba;
			//tex1Col = texture( texture01, vertUV_x2.s + waterOffset.x, vertUV_x2.t + waterOffset.y ).rgba;
			//tex2Col = texture( texture02, vertUV_x2.s + waterOffset.x, vertUV_x2.t + waterOffset.y ).rgba;
			//tex3Col = texture( texture03, vertUV_x2.s + waterOffset.x, vertUV_x2.t + waterOffset.y ).rgba;
			//tex4Col = texture( texture04, vertUV_x2.s + waterOffset.x, vertUV_x2.t + waterOffset.y ).rgba;
			//tex5Col = texture( texture05, vertUV_x2.s + waterOffset.x, vertUV_x2.t + waterOffset.y ).rgba;
			//tex6Col = texture( texture06, vertUV_x2.s + waterOffset.x, vertUV_x2.t + waterOffset.y ).rgba;
			//tex7Col = texture( texture07, vertUV_x2.s + waterOffset.x, vertUV_x2.t + waterOffset.y ).rgba;
		}
		
		if(bOffsetWater2)
		{
			vec2 newVerts = vertUV_x2.st;
			newVerts.x += waterOffset.z;
			newVerts.y += waterOffset.w;
